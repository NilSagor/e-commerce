...
abstract class Database{
	...
	private function _query($sql=null,$params=null){
		if(!empty($sql)){
			...
			if (!$statement) {
				...
				throw new Exception("Database error [{$errorInfo[0]}]:{$erorInfo[2]},driver error code is {$errorInfo[1]}");				
			}
	...
	public function getLastInsertId($sequenceName=null){
		return $this->_pdoObject->getLastInsertId($sequenceName);
	}

	public function fetchAll($sql=mull,$params=null){
		if (!empty($sql)) {
			try {
	...
	public function fetchOne($sql=null,$params=null){
		if (!empty($sql)) {
			try {
				...
				echo $this->_exceptionOutput($e,'Somethign went wrong while fetching the record');
			}
		}
		return false;
	}
	...
	public function execute($sql,$params=null){
		if (!empty($sql)) {
			try {
				...
				echo $this->_exceptionOutput($e,'Something went wrong while executing the sql statement');
			}
		}
		return false;
	}
...
return false;
	...
	public function update($table=null, $array=null,$value=null,$field='id'){
		...
		if ($this->_areUpdateParametersValid($table,$array,$value,$field)) {
			...
			$sql.="WHERE `{$field}`=?";
			$array[1][]=$value;
			return $this->execute($key,$array[1]);
		}
		return false;
	}
	...
	private function _setConnection(){
		switch($this->_schema){
			case 'mysql':
			$this->_connectionString="mysql:dbname={$this->_dbname},host={$this->_hostname}";
			break;
			...
			case 'pgsql':
			$this->_connectionString="pgsql:dbname={$this->_dbname};host={$this->_hostname}";
			break;
		}
	...
	private function _connect(){
		...
		try {
			...
			echo $this->_exceptionOutput($e,'there is a problem with the database connection');
		}
	}
