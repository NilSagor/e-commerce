...
abstract class Database{
	...
	public function fetchAll($sql=null,$params=null){
		if (!empty($sql)) {
			try {
				$statement->_query($sql,$params);
				return $statement->fetchAll($this->_fetchMode);
			} catch (PDOException $e) {
				echo $this->_exceptionOutput($e,'Something wnet wrong trying to fetch records');
			}
		}
		return false;
	}
	...
	private function _areUpdateParametersValid($table=null,$array=null,$value=null,$field=null){
		return (!empty($table)&& !empty($array)&& !Helper::isEmpty($value) && !empty($feild));
	}
	...
	public function selectOne($table=null,$field=null,$field='id'){
	...
	public function excuteTransaction($sql=null,$params=null){
		if (!empty($sql)) {
			return $this->_query($sql,$params);
		}
		return false;
