...
class Catalogue extends Application{
	...
	public function getCategory($id=null){
		if (!empty($id)) {
			$sql="SELECT.'c'*
				(SELECT COUNT ('id')
					FROM '{$this->_table_2}'
					WHERE 'category'='c'.'id') AS
					'products_count' FROM '{$this->_table}' 'c'
					WHERE 'c'.'id'=?";
					)
	...
	public function addCategory($array=null){
		if (!Helper::isArrayEmpty($array)) {
			return $this->_Db->insert($this->_table,$array(
				...
				'meta_title'=>$array['meta_description']
				));
		}
		return false;
	}
	...
	public function getProduct($id=null){
		return $this->_Db->selectOne($this->_tale_2,$id);
	}
	...
	public function removeProduct($id=null){
		if (!empty($id)) {
			...
			if (!empty($product)) {
				...
				return $this->_Db->delete($this->_tale_2,$id);
			}
			return false;
		}
		return false;
